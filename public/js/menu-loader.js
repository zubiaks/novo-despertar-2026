(async function(){
  try {
    ...
    if (!resp.ok) { container.innerHTML = '<nav class="site-nav"><a href="/index.html">Novo Despertar</a></nav>'; return; }
    ...
    if (authEl) {
      if (token) {
        authEl.innerHTML = `
          ...
          <button id="nav-logout" class="btn-link" aria-label="Logout">Logout</button>
        `;
